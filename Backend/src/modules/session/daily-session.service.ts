...
import {
  startSession as startSessionLifecycle,
  advanceToDsaInProgress,
  advanceToTechnicalInProgress,
  completeSession,
  getSessionForUser,
} from './session.service';
import { startDsaInterview, completeDsaInterview } from '@/modules/interview/dsa-interview.service';
import {
  startTechnicalInterview,
  completeTechnicalInterview,
} from '@/modules/interview/technical-interview.service';
 ...
 */
export const beginDailySession = async (
  sessionId: string,
  userId: string
): Promise<{ session: ISession; dsaInterviewId: string; problems: any[] }> => {
 ...
 */
export const finishDsaAndStartTechnical = async (
  sessionId: string
): Promise<{ session: ISession; technicalInterviewId: string; firstQuestion: string }> => {
 ...
 */
export const finishTechnicalAndCompleteSession = async (
  sessionId: string
): Promise<ISession> => {
  const session = await Session.findById(sessionId);
  if (!session) throw new AppError('Session not found', 404);
  ...
  const improvementSuggestions = [
    ...session.dsaSummary.weaknesses,
    ...technicalSummary.weaknesses,
  ].slice(0, 5); // cap it — don't dump every weakness verbatim as "improvement suggestions"
...
function extractAllStrengths(interview: any): string[] {
  return interview.questions
    .map((q: any) => q.evaluation?.strengths ?? [])
    .flat();
}

function extractAllWeaknesses(interview: any): string[] {
  return interview.questions
    .map((q: any) => q.evaluation?.weaknesses ?? [])
    .flat();
}